...
# from nd2reader import Nd2 as ND2Reader
import nd2
...
def get_imaging_channels(images):
    ...
    channels_list = images._channel_names
    number_of_channels = len(channels_list)
    channels = []
...
def run_ct_iteration(images, iteration_axis, number_of_timepoints, channels, number_of_channels):
    ...
    for ch in range(number_of_channels):
        ...
        for tm in range(number_of_timepoints):
            image_arrays[channels[ch]][tm] = {}

    for ch in range(number_of_channels):
        for tm in range(number_of_timepoints):
            # print(ch, tm)
            image_arrays[channels[ch]][tm] = np.array(images.read_frame(tm)[ch])
...
def nd2_to_array(images_path):
    ...
    images = nd2.ND2File(images_path)
    ...
    scale_x, scale_y, _ = images.metadata.channels[0].volume.axesCalibration
